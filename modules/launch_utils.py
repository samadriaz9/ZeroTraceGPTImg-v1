...
def run_pip(command, desc=None, live=default_command_live):
    ...
    # Don't add --prefer-binary for uninstall commands
    prefer_binary = '' if 'uninstall' in command else ' --prefer-binary'
    return run(f'"{python}" -m pip {command}{prefer_binary}{index_url_line}', desc=f"Installing {desc}", errdesc=f"Couldn't install {desc}", live=live)
...
def prepare_environment():
    ...
    # Ensure build tools are available before installing CLIP (which builds from source)
    if not is_installed("clip"):
        print("Ensuring build tools are available for CLIP installation...")
        # First, ensure setuptools is properly installed (use version <66 for pkg_resources support)
        print("Installing/upgrading setuptools...")
        run_pip("install --upgrade --force-reinstall 'setuptools<66'", "setuptools")
        run_pip("install --upgrade pip wheel", "build tools")
        
        # Verify pkg_resources can be imported
        print("Verifying pkg_resources availability...")
        pkg_resources_ok = check_run_python("import pkg_resources")
        if not pkg_resources_ok:
            print("pkg_resources not available, attempting to fix...")
            # Try reinstalling setuptools (skip uninstall if it fails, as setuptools might be essential)
            try:
                run_pip("uninstall -y setuptools", "uninstall setuptools", live=False)
            except RuntimeError:
                print("Could not uninstall setuptools (this is OK, continuing with reinstall)...")
            run_pip("install --force-reinstall setuptools", "reinstall setuptools")
            pkg_resources_ok = check_run_python("import pkg_resources")
        
        startup_timer.record("upgrade build tools")
        
        # Try installing CLIP
        if pkg_resources_ok:
            print("Installing CLIP (this may take a few minutes)...")
            clip_install_success = False
            installation_attempted = False
            
            # First, try installing from PyPI if available (might have pre-built wheels)
            try:
                print("Trying to install CLIP from PyPI...")
                run_pip("install ftfy regex tqdm", "CLIP dependencies", live=False)
                run_pip("install --no-build-isolation git+https://github.com/openai/CLIP.git", "clip")
                clip_install_success = True
                installation_attempted = True
            except RuntimeError:
                pass  # PyPI method failed, try zip method
            
            # If PyPI method failed, try the zip file method
            if not clip_install_success:
                try:
                    print("Trying to install CLIP from GitHub zip...")
                    ...
                    error_msg = str(e)
                    installation_attempted = True
                    if "pkg_resources" in error_msg:
                        print("="*70)
                        print("WARNING: Could not install CLIP automatically due to setuptools/pkg_resources issue.")
                        print("The application will continue, but CLIP features may not work.")
                        print("")
                        print("To fix this manually, please run these commands in PowerShell/CMD:")
                        print(f'  cd "{script_path}"')
                        print(f'  "{python}" -m pip install --upgrade --force-reinstall "setuptools<66"')
                        print(f'  "{python}" -m pip install --no-build-isolation git+https://github.com/openai/CLIP.git')
                        print("")
                        print("Or if that doesn't work:")
                        print(f'  "{python}" -m pip install {clip_package}')
                        print("="*70)
                    else:
                        raise
            
            if not clip_install_success and installation_attempted:
                print("Note: CLIP installation was skipped. The app will continue without CLIP features.")
        else:
            print("="*70)
            print("WARNING: pkg_resources is not available. Skipping CLIP installation.")
            print("The application will continue, but CLIP features may not work.")
            print("")
            print("To fix this manually, run:")
            print(f'  "{python}" -m pip install --upgrade setuptools')
            print(f'  "{python}" -m pip install {clip_package}')
            print("="*70)
    ...
    if not is_installed("open_clip"):
        # Build tools should already be upgraded, but ensure they're still available
