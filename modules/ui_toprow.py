...
class Toprow:
    ...
    def create_prompts(self):
        with gr.Column(elem_id=f"{self.id_part}_prompt_container", elem_classes=["prompt-container-compact"] if self.is_compact else [], scale=6):
            ...
            # ChatGPT Enhancement Row - Only show if API key is available
            try:
                from modules import chatgpt_integration
                chatgpt_available = chatgpt_integration.is_chatgpt_available()
            except:
                chatgpt_available = False
            
            with gr.Row(elem_id=f"{self.id_part}_chatgpt_row", elem_classes=["chatgpt-row"], visible=chatgpt_available) as chatgpt_row:
                self.chatgpt_row = chatgpt_row
            ...
            # Initialize attributes even if ChatGPT is not available (for compatibility)
            if not chatgpt_available:
                self.enhance_prompt_btn = None
                self.enhancement_percentage = None
